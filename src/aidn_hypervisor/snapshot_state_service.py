...
class SnapshotStateService:
    ...
    def snapshot_state(self) -> HypervisorStateSnapshot:
        ...
        persisted_snapshot = (
            ...
            and hasattr(self._host.state_store, "load")
            else None
        )
        return HypervisorStateSnapshot(
            ...
            runtime_protocol_connections=list(
                self._host.runtime_protocol_store.connections.values()
            ),
            runtime_protocol_ready_states=list(
                self._host.runtime_protocol_store.ready_states.values()
            ),
            runtime_protocol_health_records=list(
                self._host.runtime_protocol_store.health_records.values()
            ),
            runtime_protocol_capacity_records=list(
                self._host.runtime_protocol_store.capacity_records.values()
            ),
            runtime_protocol_messages=list(
                self._host.runtime_protocol_store.messages.values()
            ),
            runtime_protocol_sequences=dict(
                self._host.runtime_protocol_store.runtime_sequences
            ),
            runtime_protocol_requests=list(
                self._host.runtime_protocol_store.requests.values()
            ),
            runtime_protocol_cancellations=list(
                self._host.runtime_protocol_store.cancellations.values()
            ),
            runtime_protocol_cancellation_results=list(
                self._host.runtime_protocol_store.cancellation_results.values()
            ),
            runtime_protocol_results=list(
                self._host.runtime_protocol_store.results.values()
            ),
            runtime_protocol_streams=list(
                self._host.runtime_protocol_store.streams.values()
            ),
            ...
            runtime_protocol_stream_closes=list(
                self._host.runtime_protocol_store.stream_closes.values()
            ),
            runtime_protocol_artifacts=list(
                self._host.runtime_protocol_store.artifacts.values()
            ),
            runtime_protocol_state_checkpoints=list(
                self._host.runtime_protocol_store.state_checkpoints.values()
            ),
            runtime_protocol_recovery_states=list(
                self._host.runtime_protocol_store.recovery_states.values()
            ),
            runtime_protocol_usage_reports=list(
                self._host.runtime_protocol_store.usage_reports.values()
            ),
            runtime_protocol_usage_acks=list(
                self._host.runtime_protocol_store.usage_acks.values()
            ),
            runtime_protocol_usage_conflicts=list(
                self._host.runtime_protocol_store.usage_conflicts.values()
            ),
            runtime_protocol_recovery_plans=list(
                self._host.runtime_protocol_store.recovery_plans.values()
            ),
            runtime_protocol_recovery_results=list(
                self._host.runtime_protocol_store.recovery_results.values()
            ),
            runtime_protocol_drain_requests=list(
                self._host.runtime_protocol_store.drain_requests.values()
            ),
            runtime_protocol_drain_statuses=list(
                self._host.runtime_protocol_store.drain_statuses.values()
            ),
            runtime_protocol_drain_completes=list(
                self._host.runtime_protocol_store.drain_completes.values()
            ),
            runtime_protocol_shutdowns=list(
                self._host.runtime_protocol_store.shutdowns.values()
            ),
            operator_requests_policy=dict(self._host._operator_requests_policy),
            owner_wallet=(
                OwnerWalletSnapshot(**self._host._owner_wallet)
            ...
            wallet_session_events=[
                ...
                for event in self._host._wallet_session_events
            ],
            wallet_ledger_events=[
                WalletLedgerSnapshot(**event)
                for event in self._host._wallet_ledger_events
            ],
            wallet_economics_events=[
                WalletLedgerSnapshot(**event)
                for event in self._host._wallet_economics_events
            ],
            wallet_allocation_activation_events=[
                WalletAllocationActivationSnapshot(**event)
                for event in self._host._wallet_allocation_activation_events
            ],
            ...
            recyclable_removals=[
                RecyclableRemoval(**event) for event in self._host._recyclable_removals
            ],
            faucet_claims=[FaucetClaim(**event) for event in self._host._faucet_claims],
            epoch_reward_budgets=[
                EpochRewardBudget(**event) for event in self._host._epoch_reward_budgets
            ],
            endpoints=(
                [
            ...
            consumed_wallet_authorization_nonces=sorted(
                self._host._consumed_wallet_authorization_nonces
            ),
    ...
    def restore_state(self, snapshot: HypervisorStateSnapshot) -> dict[str, int]:
        ...
        self._host._consumed_wallet_authorization_nonces = set(
            snapshot.consumed_wallet_authorization_nonces
        )
        self._host._bundle_states = {
        ...
        self._host.provider_inventory = ProviderInventoryService(
            ...
            plugin_host_connections=[
                item.model_dump(mode="json")
                for item in snapshot.plugin_host_connections
            ],
        )
        ...
        for materialization in snapshot.provider_artifact_materializations:
            self._host.provider_inventory.store.save_artifact_materialization(
                materialization
            )
    ...
    def restore_runtimes(self, runtimes: list[RuntimeSnapshot]) -> None:
        ...
        for runtime in runtimes:
            ...
            if self._host._bundle_in_cooldown(runtime.bundle_id):
                ...
                recovered_runtime.last_error = self._host._current_bundle_state(
                    runtime.bundle_id
                )["cooldown_reason"]
    ...
    def _restore_wallet_sequences(self, snapshot: HypervisorStateSnapshot) -> None:
        ...
        self._host._next_wallet_usage_sequence = (
            max((event["sequence_id"] for event in self._host._wallet_usage_events), default=0)
            + 1
        )
        self._host._wallet_session_events = [
        ...
        self._host._next_faucet_claim_sequence = (
            max((event["sequence_id"] for event in self._host._faucet_claims), default=0)
            + 1
        )
        self._host._epoch_reward_budgets = [
