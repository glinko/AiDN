...
class BundleRuntimeReadinessError(RuntimeError):
    def __init__(self, readiness: dict) -> None:
        super().__init__(str(readiness.get("message") or "bundle runtime is not ready"))
...
class HypervisorService:
    ...
    def _bundle_runtime_live_gate_readiness(self, bundle: BundleConfig) -> dict:
        runtime = self._runtime_for_bundle(bundle.bundle_id)
        ...
        readiness = {
            ...
            "runtime_health_status": (
                runtime.health_status if runtime is not None else None
            ),
        }

        if not bundle.enabled:
            readiness["reason"] = "bundle_disabled"
            readiness["message"] = f"Bundle is disabled: {bundle.bundle_id}"
            return readiness

        if state["drain_mode"]:
            readiness["reason"] = "bundle_draining"
            readiness["message"] = f"Bundle is draining: {bundle.bundle_id}"
            return readiness
        ...
        if self._bundle_in_cooldown(bundle.bundle_id):
            ...
            if runtime is not None:
                ...
                runtime.last_error = state["cooldown_reason"]
                readiness["runtime_health_status"] = runtime.health_status
            readiness["reason"] = "provider_cooldown"
            readiness["message"] = f"Bundle is in cooldown: {bundle.bundle_id}"
            return readiness

        if runtime is None or runtime.status == "stopped":
            readiness["reason"] = "runtime_unavailable"
            readiness["message"] = f"Runtime is not available: {bundle.bundle_id}"
            return readiness

        plugin = self._get_plugin(bundle.plugin_id)
        if not self._health_check_with_retry(plugin, runtime, bundle.bundle_id):
            readiness["runtime_health_status"] = runtime.health_status
            readiness["reason"] = "runtime_unhealthy"
            readiness["message"] = runtime.last_error or (
                f"Runtime health check failed: {bundle.bundle_id}"
            )
            return readiness
    ...
    def bundle_runtime_readiness(self, bundle_id: str, request: TaskRequest) -> dict:
        bundle = self._get_bundle(bundle_id)
        readiness = self._bundle_runtime_live_gate_readiness(bundle)
        if not readiness["ready"]:
            return readiness

        runtime = readiness["runtime"]
        plugin = self._get_plugin(bundle.plugin_id)
        admission = self._sync_bundle_admission_readiness(
            bundle=bundle,
            plugin=plugin,
            runtime=runtime,
            request=request,
        ...
        if not admission["ready"]:
            ...
            readiness["reason"] = admission["reason"]
            readiness["message"] = admission["message"]
            readiness["estimate"] = admission["estimate"]
            return readiness
    ...
    def invoke_bundle_sync(
        ...
        *,
        ...
        cached_readiness = readiness is not None
        if readiness is None:
            readiness = self.bundle_runtime_readiness(bundle_id, request)
        if not readiness["ready"]:
            raise BundleRuntimeReadinessError(readiness)

        bundle = self._get_bundle(bundle_id)
        plugin = self._get_plugin(bundle.plugin_id)
        runtime_changed = False
        if cached_readiness:
            live_readiness = self._bundle_runtime_live_gate_readiness(bundle)
            if not live_readiness["ready"]:
                raise BundleRuntimeReadinessError(live_readiness)
            runtime = live_readiness["runtime"]
            runtime_changed = runtime.runtime_id != readiness.get("runtime_id")
            readiness.update(live_readiness)
        else:
            runtime = readiness["runtime"]
        if runtime is None:
            raise RuntimeError(f"Runtime is not available: {bundle.bundle_id}")

        estimate = readiness.get("estimate")
        if runtime_changed:
            admission = self._sync_bundle_admission_readiness(
                bundle=bundle,
                plugin=plugin,
            ...
            readiness.update(admission)
            if not admission["ready"]:
                raise BundleRuntimeReadinessError(readiness)
            estimate = admission["estimate"]
        elif not isinstance(estimate, dict):
            estimate = plugin.estimate_resources(request, bundle, runtime)
            readiness["estimate"] = estimate
        request_resources = estimate.get("request_active", {})
        request_cpu = float(request_resources.get("cpu", 0.0))
        request_ram = int(request_resources.get("ram_mb", 0))
        request_vram = int(request_resources.get("vram_mb", 0))
        request_reservation_id = None

        try:
            if self.resources is not None and (request_cpu or request_ram or request_vram):
                request_reservation_id = f"request:sync:{uuid4().hex}"
            ...
            if request_reservation_id is not None and self.resources is not None:
                self.resources.release(request_reservation_id)
            if bundle.warm_policy == "never":
                self._stop_runtime_for_bundle(bundle)
            self.process_pending()

    def _sync_bundle_admission_readiness(
        self,
        *,
        bundle: BundleConfig,
        plugin,
        ...
        estimate = plugin.estimate_resources(request, bundle, runtime)
        if self._concurrency_limit_reached(bundle=bundle, estimate=estimate):
            return {
                "ready": False,
                "reason": "concurrency_limit",
                "message": f"Bundle concurrency limit reached: {bundle.bundle_id}",
                "estimate": estimate,
            }
        ...
        if self.resources is not None and not self.resources.can_fit(
            ...
            return {
                ...
                "message": (
                    f"Insufficient request-active resources for bundle: {bundle.bundle_id}"
                ),
                "estimate": estimate,
            }
    ...
    def _effective_concurrency_limit(self, *, bundle: BundleConfig, estimate: dict) -> int:
        ...
        return effective_concurrency_limit

    def _concurrency_limit_reached(
        self,
        *,
        bundle: BundleConfig,
        estimate: dict,
        ...
        return active_tasks >= effective_concurrency_limit
    ...
    def _persist_state(self) -> None:
        ...
        persisted_snapshot = self.state_store.load()
        snapshot.endpoints = [
            manifest.model_copy(deep=True) for manifest in persisted_snapshot.endpoints
        ]
        snapshot.endpoint_configuration_snapshots = [
            configuration_snapshot.model_copy(deep=True)
            for configuration_snapshot in persisted_snapshot.endpoint_configuration_snapshots
        ]
