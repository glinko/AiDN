...
export const dashboardApi = {
  ...
  updateStewardActionPolicy: (payload: { auto_actions?: string[]; approval_actions?: string[]; max_actions_per_hour?: number; test_unrestricted?: boolean }) => writeDashboard<StewardActionPolicy>('/operators/dashboard/steward/action-policy', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  sendAgentConversationMessage: (text: string, context?: { request_id: string; surface_id: string; interaction?: unknown; chat?: unknown }) => writeDashboard<DashboardRecord>('/operators/dashboard/agent-channel/messages', { method: 'POST', body: JSON.stringify({ text, ...context }) }),
