...
export function StewardPolicyPanel({ status, policy, inference, isFetching, onRefresh, onSavePolicy, onToggle, onPrepare, onStart, onStop }: StewardPolicyPanelProps) {
  const catalog = useMemo(() => policy?.catalog ?? [], [policy?.catalog])
  const configurableCatalog = useMemo(() => catalog.filter((entry) => !entry.guard_only), [catalog])
  ...
  }, [inference?.model_path, status?.model.path, status?.execution.profile, status?.execution.ram_budget_mb, status?.execution.vram_mb])
  ...
  const isRunning = ['RUNNING', 'STARTING', 'READY'].includes(runtimeState)
  const canStart = Boolean(inference?.configured && ['READY_TO_START', 'FAILED'].includes(runtimeState))
  const lifecycleHint = isRunning
    ? 'This model is running with the shown profile and lease. Stop it before changing its path, profile, or memory budget.'
    : !inference?.configured
    ? 'Prepare the verified model before starting it.'
    : runtimeState === 'RESOURCE_WAIT'
      ? 'The host has no allocatable capacity for this budget. Lower RAM/VRAM, prepare again, then retry.'
      : runtimeState === 'READY_TO_START'
        ? 'The artifact is verified and the Resource Broker can admit it.'
        : runtimeState === 'FAILED'
          ? (inference.last_error || 'The last start failed. Prepare again after correcting the reported issue.')
          : 'Resident inference is controlled by this operator.'
  const activeLease = String(inference?.execution?.resource_lease || inference?.lease_id || '')
  return (
    <Card className="border-emerald-300/20 bg-emerald-300/[0.025] py-0 shadow-none">
      ...
      <CardContent className="space-y-5 p-4 sm:p-5">
        ...
        <section className="min-w-0 overflow-hidden rounded-xl border border-border/80 bg-[#07111d] p-4" aria-label="Steward local reasoning model">
          <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="eyebrow">Local reasoning</p><p className="mt-1 text-sm font-semibold text-white">Model lifecycle</p><p className="mt-1 max-w-3xl text-xs leading-5 text-muted-foreground">Prepare verifies the artifact and runtime profile. Start obtains a Resource Broker lease; stop releases it.</p></div><Badge variant="outline" className={cn('shrink-0 font-mono text-[10px] uppercase', isRunning ? 'border-emerald-300/30 text-emerald-100' : runtimeState === 'RESOURCE_WAIT' ? 'border-amber-300/35 text-amber-100' : 'border-slate-300/30 text-slate-300')}>{runtimeState}</Badge></div>
          <p className={cn('mt-3 break-words text-xs leading-5', runtimeState === 'RESOURCE_WAIT' || runtimeState === 'FAILED' ? 'text-amber-100' : 'text-slate-400')} role="status">{lifecycleHint}</p>
          <div className="mt-4 grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,180px)_minmax(0,150px)_minmax(0,150px)]">
            <label className="grid min-w-0 gap-1.5 md:col-span-2 xl:col-span-1"><span className="eyebrow">GGUF model path</span><input value={modelPath} disabled={isRunning} onChange={(event) => setModelPath(event.target.value)} placeholder="/var/lib/aidn/models/steward.gguf" className="min-h-11 w-full max-w-full min-w-0 rounded-lg border border-input bg-[#091725] px-3 font-mono text-xs text-white outline-none focus:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-60" /></label>
            <label className="grid min-w-0 gap-1.5"><span className="eyebrow">Execution profile</span><select value={profile} disabled={isRunning} onChange={(event) => setProfile(event.target.value as typeof profile)} className="min-h-11 w-full max-w-full min-w-0 rounded-lg border border-input bg-[#091725] px-3 text-sm text-white outline-none focus:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-60"><option value="CPU_RESIDENT">CPU resident</option><option value="IGPU_RESIDENT">iGPU resident</option><option value="GPU_RESIDENT">GPU resident</option><option value="GPU_BURST">GPU burst</option></select></label>
            <label className="grid min-w-0 gap-1.5"><span className="eyebrow">RAM budget (MB)</span><input type="number" min={128} value={ramMb} disabled={isRunning} onChange={(event) => setRamMb(event.target.value)} className="min-h-11 w-full max-w-full min-w-0 rounded-lg border border-input bg-[#091725] px-3 font-mono text-sm text-white outline-none focus:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-60" /></label>
            <label className="grid min-w-0 gap-1.5"><span className="eyebrow">VRAM budget (MB)</span><input type="number" min={0} value={vramMb} disabled={isRunning} onChange={(event) => setVramMb(event.target.value)} className="min-h-11 w-full max-w-full min-w-0 rounded-lg border border-input bg-[#091725] px-3 font-mono text-sm text-white outline-none focus:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-60" /></label>
            <label className="grid min-w-0 gap-1.5 md:col-span-2 xl:col-span-4"><span className="eyebrow">Optional HTTPS source</span><input value={sourceUrl} disabled={isRunning} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://huggingface.co/.../resolve/main/steward.gguf" className="min-h-11 w-full max-w-full min-w-0 rounded-lg border border-input bg-[#091725] px-3 font-mono text-xs text-white outline-none focus:border-emerald-300 disabled:cursor-not-allowed disabled:opacity-60" /><span className="text-xs leading-5 text-slate-400">{isRunning ? 'Configuration is locked while this resident model holds its active lease. Stop it to change model settings.' : 'When supplied, Prepare downloads atomically from the configured allow-list and verifies the artifact before the runtime can start.'}</span></label>
          </div>
          <div className="mt-4 flex min-w-0 flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 flex-wrap gap-2"><Button variant="outline" className="min-h-11 border-border bg-[#091725] text-slate-100" disabled={busy === 'prepare' || isRunning || !modelPath.trim()} title={isRunning ? lifecycleHint : undefined} onClick={() => void run('prepare', () => onPrepare({ model_path: modelPath.trim(), provider_type: 'llama.cpp', profile, ram_mb: Math.max(128, Number(ramMb) || 1024), vram_mb: Math.max(0, Number(vramMb) || 0), fallback_enabled: true, readiness_timeout_seconds: 60, ...(sourceUrl.trim() ? { source_url: sourceUrl.trim(), download: true } : {}) }), sourceUrl.trim() ? 'Model downloaded, verified, and prepared.' : 'Local model prepared and verified.')}>{busy === 'prepare' ? <Zap className="animate-pulse" /> : <Check />}{busy === 'prepare' ? 'Preparing…' : sourceUrl.trim() ? 'Download & prepare' : 'Prepare model'}</Button><Button className="min-h-11 bg-emerald-300 text-[#06121d] hover:bg-emerald-200" disabled={busy === 'start' || isRunning || !canStart} title={!canStart ? lifecycleHint : undefined} onClick={() => void run('start', onStart, 'Resident model started.')}>{busy === 'start' ? <Zap className="animate-pulse" /> : <Play />}{busy === 'start' ? 'Starting…' : 'Start model'}</Button><Button variant="outline" className="min-h-11 border-rose-300/25 bg-[#091725] text-rose-100" disabled={busy === 'stop' || !isRunning} onClick={() => void run('stop', onStop, 'Resident model stopped and its lease was released.')}>{busy === 'stop' ? <Zap className="animate-pulse" /> : <Square />}{busy === 'stop' ? 'Stopping…' : 'Stop model'}</Button></div><div className="flex shrink-0 items-center gap-2 text-xs text-slate-400"><Cpu className="size-4" />{activeLease ? `Lease ${activeLease}` : 'No active lease'}<ShieldCheck className="ml-2 size-4 text-emerald-200" />Operator-controlled</div></div>
        </section>
