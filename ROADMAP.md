...
0. Finish the public paid-MVP hardening pass on top of the new Wallet Identity
   ...
   quorum authority is accepted. The Registry now also exposes a local wallet
    ...
    authority claim.
1. Bind the verified Registry Replicator transport to persisted authenticated peer identity and controller lifecycle. The Replicator now has an opt-in fail-closed Ed25519 handshake gate with nonce replay protection for inbound and outbound exchange; bounded Plugin Release object transfer, Registry reconciliation and revocation projection also exist. Durable peer-key registration, real transport binding and automated reconciliation on reconnect remain required before making network-wide directory claims.
...
1. Persist and operate authenticated Registry peer identity through a controller lifecycle and reconnect reconciliation; the Replicator's opt-in signed handshake gate, object exchange and local untrusted metadata import are implemented, while key registration and physical transport binding remain pending.
