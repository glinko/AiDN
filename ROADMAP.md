...
1. Finish `M3` richer allocation lifecycle accounting beyond grace-based closure
2. Decide the runtime enforcement boundary for `usage_contract` and define non-token pricing
3. Define `M4` rating publication contract
4. Prepare `M5` custom model onboarding publication
5. Expand the operator dashboard from the shipped wallet-and-requests slices into deeper publish, model, and policy workflows
