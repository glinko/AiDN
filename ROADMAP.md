...
6. Provision the controlled testnet peer identity with the local Ed25519/mTLS tool, mutually import public bundles, then launch a separate replication-enabled Hypervisor instance on `hv-node10` and `node4` before collecting production Registry acceptance evidence.
