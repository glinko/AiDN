...
0. Finish the public paid-MVP hardening pass on top of the new Wallet Identity
   ...
   quorum authority is accepted. The Registry now also exposes a local wallet
    ...
    authority claim.
1. Bind the verified Registry Replicator transport to persisted authenticated peer identity and controller lifecycle. Bounded Plugin Release object transfer, Registry reconciliation and revocation projection now exist, but peer authentication, durable controller wiring and automated reconciliation on reconnect remain required before making network-wide directory claims.
...
1. Bind Plugin Release object replication to authenticated, durable Registry peer-controller lifecycle and reconnect reconciliation; object exchange and local untrusted metadata import are implemented, while peer identity remains pending.
