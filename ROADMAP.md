...
- Validation Report custody migration: deterministic compact commitments, report hashes, stable logical locators, snapshot metadata and a controlled local content-addressed report store now exist alongside legacy local reports; full report custody still duplicates compatibility data in the general Hypervisor state snapshot, submission still derives Certification in one call, and transfer receipts/challenges remain to be implemented under `docs/superpowers/plans/2026-07-18-validation-report-custody.md`;
