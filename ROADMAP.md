...
0. Finish the public paid-MVP hardening pass on top of the new Wallet Identity
   ...
   quorum authority is accepted. The Registry now also exposes a local wallet
    ...
    authority claim.
1. Bind the verified Registry Replicator transport to persisted authenticated peer identity and controller lifecycle. The Replicator now has an opt-in fail-closed Ed25519 handshake gate with nonce replay protection for inbound and outbound exchange; operator-managed peer keys persist in the Registry snapshot, survive restart, and key rotation or disablement revokes the active replication session. Bounded Plugin Release object transfer, Registry reconciliation and revocation projection also exist. Real transport binding and automated transport reconnect remain required before making network-wide directory claims.
...
1. Bind the persistent authenticated Registry peer controller to real transport connection and reconnect events; the Replicator's signed handshake gate, operator-managed peer keys, key-rotation revocation, object exchange and local untrusted metadata import are implemented, while physical transport binding remains pending.
