...
1. Extend the first non-streaming `llama.cpp` RFC-0054 executor and opt-in real Health/model/completion/Usage smoke profile into complete Runtime Adapter conformance and fault-injection scenarios, including streaming, cancellation and recovery; then apply the same suite to Ollama, vLLM-compatible and Proxy profiles.
