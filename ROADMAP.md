...
- implementation of `RFC-0055` and `RFC-0056` is now partially implemented:
  ...
  - an optional content-addressed Package Store now verifies staged package bytes against the declared SHA-256 digest and fail-closes package activation when configured. The Hypervisor now also provides a scoped Plugin Host identity/handshake contract, ephemeral install-scoped activation secrets with HMAC proof verification and nonce replay rejection, bounded JSON wire adapter, Windows Named Pipe and Unix-socket Local IPC listeners, persistent connection recovery, stale-generation revocation, constrained Provider/model/Runtime Binding control commands, and read-only operator status without credential-key disclosure. These handlers still execute inside the current Hypervisor process; activation secrets are deliberately not persisted and require a Secret Manager-backed launcher before external hosts can survive Hypervisor restart. Production package acquisition/persistence, isolated external Plugin Host process lifecycle, OCI/container lifecycle, Registry `plugin` Directory replication, full plugin update/removal/security response and RFC-0056 execution-plane conformance remain unimplemented.
