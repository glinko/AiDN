...
- implementation of `RFC-0055` and `RFC-0056` is now partially implemented:
  ...
  - an optional content-addressed Package Store now verifies staged package bytes against the declared SHA-256 digest and fail-closes package activation when configured. The Hypervisor now also provides a scoped Plugin Host identity/handshake contract, bounded JSON wire adapter, Windows Named Pipe and Unix-socket Local IPC listeners, persistent connection recovery, stale-generation revocation, constrained Provider/model/Runtime Binding control commands, and read-only operator status without credential-key disclosure. These handlers still execute inside the current Hypervisor process and use a placeholder activation-proof verifier; production package acquisition/persistence, cryptographic activation-proof verification, isolated external Plugin Host process lifecycle, OCI/container lifecycle, Registry `plugin` Directory replication, full plugin update/removal/security response and RFC-0056 execution-plane conformance remain unimplemented.

Immediate priorities:
1. Apply Runtime Protocol conformance and fault-injection scenarios to Provider Adapter profiles.
...
3. Add production package acquisition/persistence to the verified Plugin Package Store, then project signed Release metadata into the Registry `plugin` namespace.
4. Replace the in-process Plugin Host handlers with isolated package-backed process lifecycle and cryptographic activation-proof verification before enabling shell, container, download or package-manager executors.
