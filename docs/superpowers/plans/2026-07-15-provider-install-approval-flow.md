...
**Architecture:** Keep provider installation plan generation declarative, require a durable approval before apply, and bind apply to the exact approved plan/configuration hashes. The default MVP executor is `SandboxEnforcedProviderInstallationExecutor`: it validates declarative plans against a bounded non-host-mutating sandbox boundary, then creates local provider inventory state without running shell, Docker, downloads, package managers, or plugin installer code. `RecordedProviderInstallationExecutor` remains available as a simpler compatibility executor for narrower test or fallback scenarios. A narrower real host-mutating layer now exists as `ControlledFilesystemProviderInstallationExecutor`, which writes and removes installation state, prepares controlled volume directories, stages model-download manifests, stages local artifacts, extracts staged archives, promotes immutable SHA-256 model artifacts, and materializes them into provider volumes only inside one configured controlled path. Future broader host executors can replace the executor interface without changing approval, job, API, persistence, or UI contracts.

**Tech Stack:** Python 3.13, FastAPI, Pydantic v2, pytest, in-memory provider inventory store, static operator dashboard HTML/JS.

**Implementation status:** Implemented on `feat/provider-install-approval-flow` through commit `eed1286`, then extended with explicit permission acknowledgement, permission/sandbox upgrade acknowledgement, signed package verification, package-identity binding, sandbox-policy binding, executor-declared sandbox boundary compatibility, bounded declarative subset enforcement for `networks`, `health_checks`, and `resource_limits`, secret-handle selection, dry-run diagnostics, executor-level local-import readiness diagnostics, controlled local artifact staging/list/remove APIs, bounded staged-archive extraction APIs, shared Model Artifact Store promotion/deduplication, immutable multi-file Model Artifact Sets, deployment binding and reference-aware removal, plus `model-artifact://` materialization, rollback preview/status reporting, rollback execution for terminal install jobs, rollback step/timestamp persistence, a first real host-mutating controlled-filesystem executor, and a default sandbox-enforced non-host-mutating apply path. The safe MVP operator path now covers `Dry-Run -> Stage Local Artifact -> Extract Staged Archive -> Promote to Model Artifact Store -> Create Artifact Set -> Approve/Apply Provider -> Discover Models -> Bind Artifact Set -> Create Runtime Binding -> Create Endpoint Draft`, and failed or completed install jobs can now be rolled back through the same controlled inventory contract. Default apply remains non-host-mutating through `SandboxEnforcedProviderInstallationExecutor`; the new controlled-filesystem executor is opt-in and limited to one configured host path, where it prepares provider-local volume directories, staged model-download manifests, bounded local artifact imports, bounded staged-archive extraction, and immutable shared artifact materialization without performing network download, shell, or container execution. Diagnostics now surface `local-import://` and `model-artifact://` sources, whether their bytes are ready, and where each artifact will land inside the declared provider volume path. The operator surface can stage, extract, promote, list, compose basic sets, bind deployments, and remove only unreferenced artifact data without exposing arbitrary host-path access. Real shell/container/download/package-manager/plugin-installer execution is still deferred behind the existing executor, approval, job, persistence, and UI contracts.

**Next phase:** broaden host mutation beyond controlled filesystem state, with explicit operator confirmations, stronger sandbox enforcement, and more granular permission surfaces.
...
- This plan adds an `Apply` route, and the default MVP executor remains non-host-mutating.
