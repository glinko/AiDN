...
- distributed Settlement proposal, acceptance and finalization lifecycle;
- complete Runtime transport, execution events and conformance harness beyond the first durable RFC-0054 core;
...
| Session contract binding | RFC-0044 | `EndpointSession` stores `advertisement_id`, optional `offer_id`, accounting references, explicit Endpoint Payment and Consumer refund beneficiaries, deterministic `session_contract_hash`, immutable `session-contract.v2` Registry Object references, and an ordered amendment/effective-terms chain. A portable exchange package now validates and stages the base object plus amendment objects idempotently without overwriting local Session state. | Partial | Authenticated network transport, authoritative cross-node Session activation and broader distributed Contract acceptance remain incomplete. |
| Settlement evidence | RFC-0037, RFC-0060 | The canonical integer `q_atoms` core models Funding Accounts, Request Settlement Records, Input/Usage/Checkpoint roots, deterministic fallback and terminal policies, request/session caps, bounded dispute reserve, Validation zero and atomic conservation. Legacy `SESSION_SETTLE` payloads retain compatibility evidence. | Partial | Runtime Usage-to-Session bridge ingestion, durable proposal/accept/dispute stores, actual Ledger balance mutation, forced-resolution authority and correction workflow remain incomplete. |
| Accounting contract | RFC-0051, RFC-0049 | `AccountingContract` derives deterministic object metadata and now defines unavailable/partial fallback, retry/cancellation policy, Runtime Usage Profile compatibility and ceiling-bounded fixed/variable evaluation. Sessions preserve contract snapshots and object references. | Partial | Marketplace transparency projection, complete formula/rounding DSL and distributed Contract acceptance remain incomplete. |
| Usage reporting and acknowledgement | RFC-0051 | Canonical RFC-0054 evidence supports four-state Availability, per-dimension Authority, signed acknowledgments, durable chain conflict evidence and mandatory matching Final Usage before terminal Runtime transition. Accepted evidence can now be evaluated by the canonical RFC-0037 core. | Partial | Legacy-to-canonical Session bridge, network Checkpoint/correction/dispute workflow, statistical comparison and Ledger finalization remain incomplete. |
...
| Runtime binding and protocol | RFC-0053, RFC-0054 | Canonical Runtime identity/Generation/Binding hashes and Dispatcher routes exist. The durable protocol core implements externally verified handshake, version negotiation, semantic replay protection, authorized/idempotent Request admission, Usage chains/acks and explicit Recovery Plan/Result reconciliation. | Partial | Local IPC/remote transport, Ready/Health/capacity events, streaming, artifacts, Results, cancellation, checkpoints, drain/shutdown and a complete conformance harness remain missing. |
