...
| Session contract binding | RFC-0044 | `EndpointSession` now stores `advertisement_id`, optional `offer_id`, accounting references, explicit Endpoint Payment and Consumer refund beneficiaries, deterministic `session_contract_hash`, and immutable `session-contract.v2` Registry Object references. | Partial | Capability definition hash, amendment/version-chain semantics, and distributed Session Contract exchange are still missing. |
