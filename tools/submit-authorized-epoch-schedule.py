#!/usr/bin/env python3
"""Submit one signed EPOCH_SCHEDULE_COMMIT and wait for multi-RPC finality.

The command never signs an envelope and never activates a local schedule. The
input envelope must already contain the configured protocol-authority quorum.
It broadcasts identical bytes to the configured RPCs and reports success only
after operation-bound CometBFT finality verifies the exact operation type.
"""
...
def _validate_input(
    ...
    if envelope.operation_type != OPERATION_TYPE:
        raise ValueError(f"{OPERATION_TYPE} envelope is required")
    if envelope.origin_type != "protocol" or envelope.sender_wallet is not None:
        raise ValueError(f"{OPERATION_TYPE} must have protocol origin")
    if not envelope.signatures:
        raise ValueError(f"{OPERATION_TYPE} must already contain authority signatures")
    LedgerOperationService().validate_consensus_epoch_schedule_commit(envelope)
    policy.verify_epoch_schedule_commit(envelope)
...
def _broadcast_all(
    ...
    *,
    ...
    admitted = [
        ...
        if item.get("status") == "PASS" and int(item["code"]) == 0
    ]
    if not admitted:
