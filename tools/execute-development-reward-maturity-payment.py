#!/usr/bin/env python3
"""Submit one ECO-0007 maturity payment and wait for verified finality.

The executor accepts only a pre-built, source-bound
``DEVELOPMENT_REWARD_PAY_MATURITY`` envelope. It never recalculates a reward,
changes the activation scope, or applies a local balance. The exact envelope
is retained until the configured validator quorum proves finality, making a
restart safe and retries idempotent.
"""
...
def _load_or_stage_envelope(source: Path, pending: Path) -> LedgerOperationEnvelope:
    ...
    if envelope.operation_type != "DEVELOPMENT_REWARD_PAY_MATURITY":
        raise ValueError("DEVELOPMENT_REWARD_MATURITY_OPERATION_REQUIRED")
    stage = envelope.payload.get("payment_stage")
    if stage not in {"MATURITY_STAGE_ONE", "MATURITY_STAGE_TWO"}:
        raise ValueError("DEVELOPMENT_REWARD_MATURITY_STAGE_INVALID")
    amount = envelope.payload.get("amount_q_atoms")
    if isinstance(amount, bool) or not isinstance(amount, int) or amount <= 0:
        raise ValueError("DEVELOPMENT_REWARD_MATURITY_AMOUNT_INVALID")
    source_transition = envelope.payload.get("source_epoch_transition_operation_id")
    if not isinstance(source_transition, str) or re.fullmatch(r"[0-9a-f]{64}", source_transition) is None:
        raise ValueError("DEVELOPMENT_REWARD_MATURITY_SOURCE_TRANSITION_REQUIRED")
    if pending.exists():
        ...
        if existing.model_dump(mode="json") != envelope.model_dump(mode="json"):
            raise ValueError("DEVELOPMENT_REWARD_MATURITY_PENDING_CONFLICT")
    else:
...
def main() -> int:
    ...
    consensus = ConsensusService(
        ConsensusServiceConfig(
            node_id=f"development-reward-maturity:{envelope.operation_id[:16]}",
    ...
    while True:
        ...
        if finalized is not None and finalized.status == SubmissionStatus.FINALIZED:
            result = {
                "status": "FINALIZED",
                "operation_id": envelope.operation_id,
                "payment_stage": envelope.payload.get("payment_stage"),
                "amount_q_atoms": envelope.payload.get("amount_q_atoms"),
                "recipient_wallet": envelope.payload.get("recipient_wallet"),
                "reward_id": envelope.payload.get("reward_id"),
                "source_epoch_transition_operation_id": envelope.payload.get(
