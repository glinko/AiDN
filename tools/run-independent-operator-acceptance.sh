...
(cd "$evidence_dir" && sha256sum "$(basename "$registry_report")" "$(basename "$finality_report")") > "$evidence_dir/SHA256SUMS-${timestamp}"

printf '{"status":"ok","registry_report":"%s","finality_report":"%s","ownership_evidence":"NOT_PROVEN_BY_PROTOCOL"}\n' \
  "$registry_report" "$finality_report"
