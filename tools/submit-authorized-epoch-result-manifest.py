#!/usr/bin/env python3
"""Submit a signed EPOCH_RESULT_MANIFEST_COMMIT and await multi-RPC finality.

The command never calculates roots, creates budgets or signs the envelope. It
only submits an already reviewed threshold-authorized commitment and reports
...
from aidn_hypervisor.consensus.epoch_result_manifest import (  # noqa: E402
...
def _validate_input(envelope: LedgerOperationEnvelope, policy: ProtocolAuthorityPolicy) -> None:
    if envelope.operation_type != EPOCH_RESULT_MANIFEST_OPERATION:
        raise ValueError("EPOCH_RESULT_MANIFEST_COMMIT envelope is required")
    if envelope.origin_type != "protocol" or envelope.sender_wallet is not None:
        raise ValueError("EPOCH_RESULT_MANIFEST_COMMIT must have protocol origin")
    if not envelope.signatures:
        raise ValueError("EPOCH_RESULT_MANIFEST_COMMIT must already contain authority signatures")
    LedgerOperationService().validate_consensus_epoch_result_manifest(envelope)
    policy.verify_epoch_result_manifest_commit(envelope)
...
def _broadcast_all(
    ...
    *,
    ...
    if not any(item.get("status") == "PASS" and item.get("code") == 0 for item in observations):
...
def main() -> int:
    ...
    policy_value = json.loads(args.policy.read_text(encoding="utf-8"))
    if not isinstance(policy_value, dict):
        raise ValueError("policy must contain a JSON object")
    policy = ProtocolAuthorityPolicy.from_mapping(policy_value)
    deployment = load_cometbft_finality_deployment_config(args.finality_config)
    _validate_input(envelope, policy)
    transaction_hash = cometbft_transaction_hash(envelope.consensus_bytes())
    summary: dict[str, Any] = {
        "operation_id": envelope.operation_id,
        "operation_type": EPOCH_RESULT_MANIFEST_OPERATION,
        "epoch_number": envelope.target_epoch,
    ...
    summary["submissions"] = _broadcast_all(
        envelope,
        endpoints,
        timeout_seconds=args.timeout_seconds,
    )
    ...
    finality_source = build_cometbft_multi_rpc_finality_source(
        ...
        transaction_hash_for_operation={envelope.operation_id: transaction_hash}.get,
    )
    deadline = time.monotonic() + args.timeout_seconds
    while time.monotonic() < deadline:
        evidence = finality_source.finality_evidence(envelope.operation_id)
        if evidence is not None:
            if evidence.operation_type != EPOCH_RESULT_MANIFEST_OPERATION or evidence.chain_id != deployment.chain_id:
                raise ValueError("EPOCH_RESULT_MANIFEST_COMMIT finality evidence does not match input")
