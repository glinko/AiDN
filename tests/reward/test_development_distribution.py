...
def test_low_demand_keeps_uncommitted_pool_as_carryover():
    ...
    # ECO-0010 assigns 60% of 5000Q; class reserves remain inside that pool.
    assert calculation.pool.base_allocation_q_atoms == 3_000_000_000
    assert calculation.pool.available_contribution_budget_q_atoms == 2_400_000_000
    assert calculation.allocations[0].accepted_reward_q_atoms == 10_000_000
    assert calculation.pool.carryover_out_q_atoms == 2_390_000_000
...
def test_oversubscribed_demand_normalizes_deterministically():
    contributions = [_contribution(f"c-{index}", units_milli=1_000_000) for index in range(6)]
    ...
    assert first.nominal_demand_q_atoms == 3_600_000_000
    assert first.target_contribution_budget_q_atoms == 2_400_000_000
    assert first.normalization_factor_millionths == 666_666
    assert sum(item.accepted_reward_q_atoms for item in first.allocations) == 2_400_000_000
...
def test_contributor_cap_defers_excess_instead_of_redistributing_it():
    ...
    # Base=3000Q, automatic contributor cap=35%=1050Q.
    assert calculation.accepted_gross_reward_q_atoms == 1_050_000_000
    assert calculation.contributor_cap_overflow_q_atoms == 150_000_000
    assert calculation.pool.carryover_out_q_atoms == 1_350_000_000
    assert sum(item.accepted_reward_q_atoms for item in calculation.allocations) == 1_050_000_000
...
def test_contribution_group_shares_one_logical_cap():
    ...
    assert calculation.nominal_demand_q_atoms == 600_000_000
    assert calculation.accepted_gross_reward_q_atoms == 600_000_000
    assert calculation.group_cap_overflow_q_atoms == 1_800_000_000
    assert calculation.pool.carryover_out_q_atoms == 1_800_000_000
...
def test_reserved_maturity_and_bounty_reduce_available_budget():
    ...
    assert calculation.pool.available_contribution_budget_q_atoms == 2_350_000_000
    assert calculation.pool.maturity_reserve_out_q_atoms == 80_000_000
    assert calculation.total_accounted_q_atoms == calculation.pool.pool_in_q_atoms
...
def test_invalid_pool_reserve_and_invalid_policy_are_rejected():
    ...
    with pytest.raises(ValueError, match="DEVELOPMENT_POOL_INSUFFICIENT"):
        DevelopmentRewardCalculator().calculate(
            _pool(
                maturity_reserve_in_q_atoms=2_000_000_000,
                approved_bounty_reservations_q_atoms=1_000_000_000,
            ),
            [],
        )
