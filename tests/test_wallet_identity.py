...
def _registered_identity(service: HypervisorService, wallet_id: str = "wallet-consumer"):
    ...
    signature = private_key.sign(
        wallet_identity_registration_payload(wallet_id=wallet_id, public_key=public_key, registration_nonce=nonce)
    ).hex()
    service.register_wallet_identity(
        wallet_id=wallet_id, public_key=public_key, registration_nonce=nonce, signature=f"ed25519:{signature}"
    )
...
def test_signed_session_open_locks_escrow_once_per_authorization_nonce() -> None:
    ...
    endpoint = (
        EndpointService(EndpointStore())
        .create_endpoint(
            CreateEndpointCommand(
                owner_wallet="wallet-operator",
                bundle_id="bundle-1",
                bundle_hash="bundle-hash",
                display_name="Paid endpoint",
